...
class KGEModel(nn.Module):
    def __init__(self, model_name, nentity, nrelation, hidden_dim, gamma, evaluator,
        ...
        if model_name not in ['TransE', 'DistMult', 'ComplEx', 'RotatE', 'PairRE', 'RotatEv2', 'CompoundE', 'CompoundE_Head']:
        ...
        if model_name in ['CompoundE', 'CompoundE_Head'] and (not triple_relation_embedding):
            raise ValueError('CompoundE/CompoundE_Head should use --triple_relation_embedding')
    ...
    def forward(self, sample, mode='single'):
        ...
        model_func = {
            ...
            'CompoundE_Head': self.CompoundE_Head
        }
    ...
    def CompoundE_Head(self, head, relation, tail, mode):
        head_scale, head_translate, theta = torch.chunk(relation, 3, dim=2)
        ...
        # 1. Scale
        head_s = head * head_scale

        # 2. Rotate
        head_s = head_s.view((head_s.shape[0], head_s.shape[1], -1, 2))
        re_head = head_s[:, :, :, 0:1]
        im_head = head_s[:, :, :, 1:2]

        head_r_re = re_head * re_rotation - im_head * im_rotation
        head_r_im = re_head * im_rotation + im_head * re_rotation
        
        head_r = torch.cat([head_r_re, head_r_im], dim=-1)
        head_r = head_r.view((head_r.shape[0], head_r.shape[1], -1))

        # 3. Translate
        head_prime = head_r + head_translate

        score = head_prime - tail
        score = self.gamma.item() - torch.norm(score, p=1, dim=2)
        return score

    def get_structural_embeddings(self, sample):
        # sample is (batch_size, 3) -> [h, r, t]
        head = torch.index_select(
            self.entity_embedding,
            dim=0,
            index=sample[:, 0]
        ...
        re_rotation = torch.cos(theta).unsqueeze(-1)
        im_rotation = torch.sin(theta).unsqueeze(-1)
        ...
        # Return (h', t)
        return head_prime.squeeze(1), tail.squeeze(1)
